...
sli_zigbee_af_scan_dispatch_energy_scan_result_callback(channel, maxRssiValue);
(void)channel;
(void)maxRssiValue;
}
...
sli_zigbee_af_scan_dispatch_network_found_callback(networkFound, lqi, rssi);
(void)networkFound;
(void)lqi;
(void)rssi;
}
...
sli_zigbee_af_scan_dispatch_scan_complete_callback(channel, status);
(void)channel;
(void)status;
}
...
sl_zigbee_af_network_creator_security_zigbee_key_establishment_cb(partner, status);
...
sli_zigbee_af_interpan_mac_filter_match_message_callback (filterValueMatch, messageType, packetInfo, messageLength, messageContents);
...
sli_zigbee_af_green_power_client_gpep_incoming_message_callback(params);
(void)params;
}
...
// No arguments to void-cast for this function
}
