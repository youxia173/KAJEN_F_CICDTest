...
#if (SL_ZIGBEE_AF_PLUGIN_COLOR_CONTROL_SERVER_XY == 1)
#define SUPPORT_CIE_1931
#endif
#if (SL_ZIGBEE_AF_PLUGIN_COLOR_CONTROL_SERVER_TEMP == 1)
#define SUPPORT_COLOR_TEMPERATURE
#endif
#if (SL_ZIGBEE_AF_PLUGIN_COLOR_CONTROL_SERVER_HSV == 1)
#define SUPPORT_HUE_SATURATION
#endif
sl_zigbee_af_event_t sl_zigbee_af_color_control_server_temp_transition_event;
sl_zigbee_af_event_t sl_zigbee_af_color_control_server_xy_transition_event;
sl_zigbee_af_event_t sl_zigbee_af_color_control_server_hue_sat_transition_event;

#define COLOR_TEMP_CONTROL (&sl_zigbee_af_color_control_server_temp_transition_event)
#define COLOR_XY_CONTROL   (&sl_zigbee_af_color_control_server_xy_transition_event)
#define COLOR_HSV_CONTROL  (&sl_zigbee_af_color_control_server_hue_sat_transition_event)

static void tempTransitionEventHandler(sl_zigbee_af_event_t * event);
static void xyTransitionEventHandler(sl_zigbee_af_event_t * event);
static void hueSatTransitionEventHandler(sl_zigbee_af_event_t * event);
...
typedef struct {
  ...
  bool     up;
  bool     repeat;
} sli_zigbee_color_hue_transition_state_t;

static sli_zigbee_color_hue_transition_state_t colorHueTransitionState;
...
typedef struct {
  ...
  uint16_t highLimit;
  uint8_t  endpoint;
} sli_zigbee_color_16u_transistion_state_t;

static sli_zigbee_color_16u_transistion_state_t colorXTransitionState;
static sli_zigbee_color_16u_transistion_state_t colorYTransitionState;

static sli_zigbee_color_16u_transistion_state_t colorTempTransitionState;

static sli_zigbee_color_16u_transistion_state_t colorSaturationTransitionState;

// Forward declarations:
static bool computeNewColor16uValue(sli_zigbee_color_16u_transistion_state_t *p);
...
static uint16_t computeTransitionTimeFromStateAndRate(sli_zigbee_color_16u_transistion_state_t *p,
                                                      uint16_t rate);
...
void sli_zigbee_af_color_control_server_init_callback(uint8_t init_level)
{
  ...
  sl_zigbee_af_event_init(&sl_zigbee_af_color_control_server_temp_transition_event,
                          tempTransitionEventHandler);
  sl_zigbee_af_event_init(&sl_zigbee_af_color_control_server_xy_transition_event,
                          xyTransitionEventHandler);
  sl_zigbee_af_event_init(&sl_zigbee_af_color_control_server_hue_sat_transition_event,
...
sl_zigbee_af_zcl_request_status_t sl_zigbee_af_color_control_cluster_move_hue_cb(sl_zigbee_af_cluster_command_t *cmd)
{
  ...
  if (moveMode == SL_ZIGBEE_ZCL_HUE_MOVE_MODE_UP) {
    colorHueTransitionState.finalHue = addHue(readHue(endpoint),
                                              rate);
    colorHueTransitionState.up = true;
  } else if (moveMode == SL_ZIGBEE_ZCL_HUE_MOVE_MODE_DOWN) {
...
sl_zigbee_af_zcl_request_status_t sl_zigbee_af_color_control_cluster_move_saturation_cb(sl_zigbee_af_cluster_command_t *cmd)
{
  ...
  colorSaturationTransitionState.initialValue = readSaturation(endpoint);
  colorSaturationTransitionState.currentValue = readSaturation(endpoint);
  if (moveMode == SL_ZIGBEE_ZCL_SATURATION_MOVE_MODE_UP) {
...
void sl_zigbee_af_level_control_coupled_color_temp_change_cb(uint8_t endpoint)
{
  ...
  if (!sl_zigbee_af_contains_server(endpoint, ZCL_COLOR_CONTROL_CLUSTER_ID)) {
    return;
  }
...
static void stopAllColorTransitions(void)
{
  sl_zigbee_af_event_set_inactive(COLOR_TEMP_CONTROL);
  sl_zigbee_af_event_set_inactive(COLOR_XY_CONTROL);
  sl_zigbee_af_event_set_inactive(COLOR_HSV_CONTROL);
}
                             ...
                             uint8_t newColorMode)
{
  ...
  switch (colorModeTransition) {
    case HSV_TO_CIE_XY:
      sl_zigbee_af_color_control_server_compute_pwm_from_xy_cb(endpoint);
      break;
    case TEMPERATURE_TO_CIE_XY:
      sl_zigbee_af_color_control_server_compute_pwm_from_xy_cb(endpoint);
      break;
    case CIE_XY_TO_HSV:
      sl_zigbee_af_color_control_server_compute_pwm_from_hsv_cb(endpoint);
      break;
    case TEMPERATURE_TO_HSV:
      sl_zigbee_af_color_control_server_compute_pwm_from_hsv_cb(endpoint);
      break;
    case HSV_TO_TEMPERATURE:
      sl_zigbee_af_color_control_server_compute_pwm_from_temp_cb(endpoint);
      break;
...
static bool computeNewHueValue(sli_zigbee_color_hue_transition_state_t *p)
{
  uint32_t newHue32;
  uint8_t newHue;
...
static void hueSatTransitionEventHandler(sl_zigbee_af_event_t * event)
{
  ...
  sl_zigbee_af_color_control_cluster_println("Hue %d Saturation %d endpoint %d",
                                             colorHueTransitionState.currentHue,
...
static void xyTransitionEventHandler(sl_zigbee_af_event_t * event)
{
  ...
  sl_zigbee_af_color_control_cluster_println("Color X %d Color Y %d",
                                             colorXTransitionState.currentValue,
                                             colorYTransitionState.currentValue);
...
static void tempTransitionEventHandler(sl_zigbee_af_event_t * event)
{
  ...
  sl_zigbee_af_color_control_cluster_println("Color Temperature %d",
                                             colorTempTransitionState.currentValue);
                               ...
                               uint8_t optionOverride)
{
  ...
  if (optionMask == 0xFF && optionOverride == 0xFF) {
    ...
    return READBITS(options, SL_ZIGBEE_ZCL_COLOR_CONTROL_OPTIONS_EXECUTE_IF_OFF);
  }
  // ---------- The above is to distinguish if the payload is present or not

  if (READBITS(optionMask, SL_ZIGBEE_ZCL_COLOR_CONTROL_OPTIONS_EXECUTE_IF_OFF)) {
    // Mask is present and set in the command payload, this indicates
    // use the override as temporary option
    return READBITS(optionOverride, SL_ZIGBEE_ZCL_COLOR_CONTROL_OPTIONS_EXECUTE_IF_OFF);
  }
    ...
    if (status == SL_ZIGBEE_ZCL_STATUS_SUCCESS) {
      ...
      if (tempPhysicalMin <= startUpColorTemp && startUpColorTemp <= tempPhysicalMax) {
        ...
        if (status == SL_ZIGBEE_ZCL_STATUS_SUCCESS) {
          ...
          uint8_t updateColorMode = SL_ZIGBEE_ZCL_COLOR_MODE_COLOR_TEMPERATURE;
          status = sl_zigbee_af_write_attribute(endpoint,
                                                ZCL_COLOR_CONTROL_CLUSTER_ID,
                                                ZCL_COLOR_CONTROL_COLOR_MODE_ATTRIBUTE_ID,
                                                CLUSTER_MASK_SERVER,
          ...
          updateColorMode = SL_ZIGBEE_ZCL_ENHANCED_COLOR_MODE_COLOR_TEMPERATURE;
          status = sl_zigbee_af_write_attribute(endpoint,
                                                ZCL_COLOR_CONTROL_CLUSTER_ID,
                                                ZCL_COLOR_CONTROL_ENHANCED_COLOR_MODE_ATTRIBUTE_ID,
                                                CLUSTER_MASK_SERVER,
