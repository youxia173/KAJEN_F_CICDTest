 ...
 * and there are SL_ZIGBEE_GP_PROXY_TABLE_SIZE/SL_ZIGBEE_GP_SINK_TABLE_SIZE many of them.<br>
 *<br>
 ...
 * operations, are performed using the ::sl_zigbee_sec_man_load_key_context routine.
 *
 ...
 * (SL_ZIGBEE_TRUST_CENTER_USES_HASHED_LINK_KEY), or if SL_ZIGBEE_TRUST_CENTER_GLOBAL_LINK_KEY is set.
 *
 ...
 * ::SL_ZIGBEE_TRANSIENT_KEY_TIMEOUT_S seconds, unless the key entry was added using
 * the Network Creator Security component, in which case the key will time out
 * after the longer between
 * ::SL_ZIGBEE_AF_PLUGIN_NETWORK_CREATOR_SECURITY_NETWORK_OPEN_TIME_S seconds and
 * ::SL_ZIGBEE_TRANSIENT_KEY_TIMEOUT_S seconds.
 *
 * @param eui64 [IN] An sl_802154_long_addr_t to import.
 * @param key [IN] A sl_zigbee_sec_man_key_t* to import.
 *
 ...
 *  SL_ZIGBEE_HAVE_PRECONFIGURED_KEY and
 *  SL_ZIGBEE_GET_PRECONFIGURED_KEY_FROM_INSTALL_CODE have not been set in the
 ...
 * ::sl_zigbee_sec_man_export_key for the requirements of the context argument based
 * on core_key_type.
 *
 ...
 *
 * @internal SL_ZIGBEE_IPC_ARGS
 * {# input | length: data_length | max: MAX_IPC_VEC_ARG_CAPACITY #}
 * {# output | length: 16 | max: MAX_IPC_VEC_ARG_CAPACITY #}
 */
...
sl_status_t sl_zigbee_sec_man_aes_ccm(uint8_t* nonce,
                                      bool encrypt,
                                      const uint8_t* input,
                                      uint8_t encryption_start_index,
                                      uint8_t length,
                                      uint8_t* output);
#else
#define sl_zigbee_sec_man_aes_ccm(nonce, encrypt, input, encryption_start_index, length, output) \
  sl_zigbee_sec_man_aes_ccm_extended(nonce, encrypt, input, encryption_start_index, length, 4, output)
#endif
/**
 ...
 *
 * @internal SL_ZIGBEE_IPC_ARGS
 * {# input | length: 16 | max: 16 #}
 * {# output | length: 16 | max: 16 #}
 */
 ...
 */
#define sl_zigbee_sec_man_symmetric_passphrase_update_allowed(address) \
  (sl_zigbee_sec_man_find_symmetric_passphrase_key_table_index(address) == 0xFF)
...
/** @brief Get the install-code derived key on the device.
 *
 * This function will query the stack for the install code. The stack will
 * convert the install code to a 16 byte key and return that key.
 *
 * @param key  A pointer to the key to fill in.
 *
 * @return ::SL_STATUS_OK on success, ::SL_STATUS_NOT_AVAILABLE
 *   if the install code library is not included on the SoC/NCP,
 *   or ::SL_STATUS_INVALID_CONFIGURATION if the install code
 *   has not been written into its relevant token.
 */
...
sl_status_t slx_zigbee_set_key_table_additional_info(uint8_t index, uint32_t tok);
sl_status_t slx_zigbee_get_key_table_additional_info(uint8_t index, uint32_t *tok);
