...
NO_INIT(uint32_t gLastHalSysResetCause);
NO_INIT(uint32_t gLastHalSysResetSeen);
NO_INIT(uint32_t gLastHalSysResetMagic);

#define HAL_SYSRESET_DIAG_MAGIC 0x48535253UL
...
void halInternalSysReset(uint16_t extendedCause)
{
  ...
  if (gLastHalSysResetMagic != HAL_SYSRESET_DIAG_MAGIC) {
    gLastHalSysResetMagic = HAL_SYSRESET_DIAG_MAGIC;
    gLastHalSysResetSeen = 0;
    gLastHalSysResetCause = 0;
  }

  gLastHalSysResetCause = extendedCause;
  gLastHalSysResetSeen++;
