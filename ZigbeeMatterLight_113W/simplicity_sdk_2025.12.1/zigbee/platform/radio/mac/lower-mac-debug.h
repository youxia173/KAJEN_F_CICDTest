...
extern sl_status_t sli_legacy_serial_guaranteed_printf(uint8_t port,
                                        ...
                                        uint8_t isFlag)
{
  ...
  halResetWatchdog();
  while (string[j] != '\0') {
    sli_legacy_serial_guaranteed_printf(ACTIONS_DUMP_SERIAL_PORT, "%c", string[j]);
    j++;
  }
  if (isFlag) {
    flagCharsPrinted += j;
  ...
  if (fixWidth) {
    sli_legacy_serial_guaranteed_printf(ACTIONS_DUMP_SERIAL_PORT, " ");
    while (j < PRINT_GENERIC_FIELD_LENGTH) {
      sli_legacy_serial_guaranteed_printf(ACTIONS_DUMP_SERIAL_PORT, " ");
      j++;
    }
