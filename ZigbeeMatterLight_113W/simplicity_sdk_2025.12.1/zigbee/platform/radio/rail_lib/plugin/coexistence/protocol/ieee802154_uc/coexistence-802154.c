...
#if !defined SL_RAIL_UTIL_COEX_IEEE802154_SIGNAL_IDENTIFIER_ENABLED
...
static bool is_coex_req_active(void)
{
  return txReq.coexReq != COEX_REQ_OFF
         || rxReq.coexReq != COEX_REQ_OFF;
}
...
#endif // SL_RAIL_UTIL_COEX_PRIORITY_ESCALATION_ENABLE || defined(DEBUG_PTA)
  txrxReq = COEX_EVENT_TX_STATE;
  status = internalPtaSetRequest(&txReq, coexReq, coexCb);
  return status;
}
...
sl_status_t sl_rail_util_coex_set_rx_request(COEX_Req_t coexReq, COEX_ReqCb_t coexCb)
{
  ...
  status = internalPtaSetRequest(&rxReq, coexReq, coexCb);
  return status;
}
