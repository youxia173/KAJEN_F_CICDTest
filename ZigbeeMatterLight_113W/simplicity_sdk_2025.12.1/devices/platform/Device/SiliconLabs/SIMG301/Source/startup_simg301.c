...
volatile uint32_t gEarlyEmuResetCause __attribute__((section(".noinit")));
volatile uint32_t gEarlyEmuResetCauseSeen __attribute__((section(".noinit")));
...
__NO_RETURN void Reset_Handler(void)
{
  ...
  // Capture the hardware reset-cause register before platform layers clear it.
  gEarlyEmuResetCause = EMU->RSTCAUSE;
  gEarlyEmuResetCauseSeen++;
