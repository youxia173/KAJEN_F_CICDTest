...
typedef struct mbedtls_pk_context {
...
#else
    uint32_t MBEDTLS_PRIVATE(priv_id);                  /**< Key ID for opaque keys */
     ...
     *       context.
     */
    uint8_t MBEDTLS_PRIVATE(pub_raw)[66 * 2 + 1];   /**< Raw public key   */
...
#else
    uint8_t MBEDTLS_PRIVATE(ec_family);             /**< EC family of pk */
