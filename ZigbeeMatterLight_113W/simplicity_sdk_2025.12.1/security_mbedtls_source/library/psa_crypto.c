    ...
    size_t *key_buffer_length, size_t *bits)
{
    ...
    if (key_type_is_raw_bytes(type)) {
        ...
        if (PSA_BITS_TO_BYTES(*bits) > key_buffer_size) {
            return PSA_ERROR_BUFFER_TOO_SMALL;
        }

        memcpy(key_buffer, data, PSA_BITS_TO_BYTES(*bits));
        *key_buffer_length = PSA_BITS_TO_BYTES(*bits);
...
psa_status_t psa_destroy_key(mbedtls_svc_key_id_t key)
{
    ...
    overall_status = psa_driver_wrapper_destroy_key(&slot->attr);
    if (overall_status != PSA_SUCCESS) {
        goto exit;
    }
...
{
    return psa_driver_wrapper_validate_key_usage(policy);
}
                          ...
                          mbedtls_svc_key_id_t *target_key)
{
    ...
    bool source_key_is_unconditionally_copyable = false;

    *target_key = MBEDTLS_SVC_KEY_ID_INIT;

    status = psa_get_and_lock_key_slot_with_policy(
        source_key, &source_slot, 0, 0);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    source_key_is_unconditionally_copyable =
      sli_psa_key_is_unconditionally_copyable(source_slot->attr.id);

    if (!source_key_is_unconditionally_copyable &&
        ((source_slot->attr.policy.usage & PSA_KEY_USAGE_COPY)
         != PSA_KEY_USAGE_COPY)) {
        status = PSA_ERROR_NOT_PERMITTED;
        goto exit;
    }
    ...
    target_location =
        PSA_KEY_LIFETIME_GET_LOCATION(psa_get_key_lifetime(&target_slot->attr));
    if (target_location != PSA_KEY_LOCATION_LOCAL_STORAGE) {
        status = psa_driver_wrapper_get_key_buffer_size(&target_slot->attr, &storage_size);
        if (status != PSA_SUCCESS) {
            goto exit;
        }
        ...
        status = psa_driver_wrapper_copy_key(&source_slot->attr,
                                             &target_slot->attr,
    ...
    } else if (source_key_is_unconditionally_copyable &&
               psa_key_lifetime_is_external(source_slot->attr.lifetime)) {
      /* We can't copy from an opaque key into a transparent one. */
      status = PSA_ERROR_NOT_PERMITTED;
      goto exit;
    } else {
    ...
    mbedtls_svc_key_id_t *key)
{
    ...
    *key = MBEDTLS_SVC_KEY_ID_INIT;
    ...
    if (psa_get_key_bits(attributes) == 0 ||
        psa_get_key_bits(attributes) % 8 != 0) {
        return PSA_ERROR_INVALID_ARGUMENT;
    }
    if (!key_type_is_raw_bytes(psa_get_key_type(attributes))) {
        return PSA_ERROR_INVALID_ARGUMENT;
    }
                                     ...
                                     mbedtls_svc_key_id_t *key)
{
    ...
    *key = MBEDTLS_SVC_KEY_ID_INIT;
    ...
    status = psa_driver_wrapper_generate_key(&slot->attr,
                                             custom,
                                             custom_data, custom_data_length,
                                             slot->key.data, slot->key.bytes,
                                             &slot->key.bytes);
...
#if defined(SLI_MBEDTLS_PSA_EC_JPAKE_TLS_WORKAROUND)

psa_status_t psa_pake_derive_secret(
    psa_pake_operation_t *operation,
    uint8_t *key_buf,
    size_t key_length)
{
    ...
    ret = mbedtls_ecjpake_derive_secret(
        &(&operation->data.ctx.mbedtls_ctx)->ctx.jpake,
        key_buf,
        key_length,
        &key_output_length,
        mbedtls_psa_get_random,
    ...
    if (key_output_length != key_length ||
        ret != 0) {
...
#endif // SLI_MBEDTLS_PSA_EC_JPAKE_TLS_WORKAROUND
