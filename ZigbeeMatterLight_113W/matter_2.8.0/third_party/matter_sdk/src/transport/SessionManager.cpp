                                                  ...
                                                  Transport::MessageTransportContext * ctxt)
{
    ...
    if (SecureMessageCodec::Decrypt(secureSession->GetCryptoContext(), nonce, payloadHeader, packetHeader, msg) != CHIP_NO_ERROR)
    {
        ChipLogError(
            Inet,
            "Secure transport failed to decode/authenticate msg, drop: lsid=%u psid=%u hdr_sid=%u mc=" ChipLogFormatMessageCounter
            " type=0x%02x state=%s",
            secureSession->GetLocalSessionId(), secureSession->GetPeerSessionId(), packetHeader.GetSessionId(),
            packetHeader.GetMessageCounter(), payloadHeader.GetMessageType(), secureSession->GetStateStr());
        return;
    }
