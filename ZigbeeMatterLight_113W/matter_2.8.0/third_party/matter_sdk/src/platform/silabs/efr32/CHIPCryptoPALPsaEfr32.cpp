                           ...
                           uint8_t * tag, size_t tag_length)
{
    ...
    else
    {
        ...
        VerifyOrReturnError(block_aligned_length == PSA_AEAD_UPDATE_OUTPUT_SIZE(PSA_KEY_TYPE_AES, algorithm, block_aligned_length),
                            CHIP_ERROR_INTERNAL,
                            ChipLogError(Crypto, "block_aligned_length is not compliant with PSA's output size requirements"));
        ...
        if (partial_block_length > 0)
        {
            ...
            VerifyOrReturnError(max_output <= sizeof(temp), CHIP_ERROR_INTERNAL,
                                ChipLogError(Crypto, "psa_aead_update output is too large"));
        ...
        VerifyOrReturnError(ciphertext_length == plaintext_length, CHIP_ERROR_INTERNAL,
                            ChipLogError(Crypto, "Encrypted ciphertext length does not match plaintext length"));
    }
                           ...
                           size_t nonce_length, uint8_t * plaintext)
{
    ...
    else
    {
        ...
        plaintext_length += out_length;
        VerifyOrReturnError(ciphertext_length == plaintext_length, CHIP_ERROR_INTERNAL,
                            ChipLogError(Crypto, "Decrypted plaintext length does not match ciphertext length"));
    }
    VerifyOrReturnError(status == PSA_SUCCESS, CHIP_ERROR_INTERNAL);
...
static_assert(sizeof(Spake2p_Context) <= sizeof(Spake2pOpaqueContext), "Spake2p_Context exceeds opaque storage size");
...
CHIP_ERROR Spake2p_P256_SHA256_HKDF_HMAC::PointMul(void * R, const void * P1, const void * fe1)
{
    ...
    uint8_t serializedPoint[1 + 2 * kP256_FE_Length] = { 0 };
    size_t serializedPointLen = 0;
    ...
    // Normalize P1 to affine coordinates before extracting X/Y.
    int pointWriteResult = mbedtls_ecp_point_write_binary(&context->curve, (const mbedtls_ecp_point *) P1,
                                                          MBEDTLS_ECP_PF_UNCOMPRESSED, &serializedPointLen,
                                                          serializedPoint, sizeof(serializedPoint));
    if (pointWriteResult != 0)
    {
        ChipLogError(Crypto, "ECDH point serialization failed: %d", pointWriteResult);
        return CHIP_ERROR_INVALID_ARGUMENT;
    }

    if (serializedPointLen != sizeof(serializedPoint) || serializedPoint[0] != 0x04)
    {
        ChipLogError(Crypto, "ECDH point serialization produced invalid format len=%lu tag=0x%02x",
                     static_cast<unsigned long>(serializedPointLen), serializedPoint[0]);
        return CHIP_ERROR_INVALID_ARGUMENT;
    }

    memcpy(point, serializedPoint + 1, sizeof(point));

    mbedtls_ecp_point normalizedPoint;
    mbedtls_ecp_point_init(&normalizedPoint);

    int loadPointResult = mbedtls_ecp_point_read_binary(&context->curve, &normalizedPoint, serializedPoint, serializedPointLen);
    if (loadPointResult != 0)
    {
        ChipLogError(Crypto, "ECDH normalized point load failed: %d", loadPointResult);
        mbedtls_ecp_point_free(&normalizedPoint);
        return CHIP_ERROR_INVALID_ARGUMENT;
    }

    auto fallbackToMbedtls = [&](const char * reason, long statusCode) -> CHIP_ERROR {
        int result = 0;
        int check  = 0;
        mbedtls_mpi scalar;
        mbedtls_mpi_init(&scalar);

        check = mbedtls_ecp_check_pubkey(&context->curve, &normalizedPoint);
        if (check != 0)
        {
            ChipLogError(Crypto, "ECDH fallback (%s, status=%ld) invalid public key: %d", reason, statusCode, check);
            mbedtls_mpi_free(&scalar);
            return CHIP_ERROR_INVALID_ARGUMENT;
        }

        result = mbedtls_mpi_copy(&scalar, (const mbedtls_mpi *) fe1);
        if (result != 0)
        {
            ChipLogError(Crypto, "ECDH fallback (%s, status=%ld) mpi_copy=%d", reason, statusCode, result);
            mbedtls_mpi_free(&scalar);
            return CHIP_ERROR_INTERNAL;
        }

        result = mbedtls_mpi_mod_mpi(&scalar, &scalar, &context->curve.N);
        if (result != 0)
        {
            ChipLogError(Crypto, "ECDH fallback (%s, status=%ld) scalar mod N=%d", reason, statusCode, result);
            mbedtls_mpi_free(&scalar);
            return CHIP_ERROR_INTERNAL;
        }

        check = mbedtls_ecp_check_privkey(&context->curve, &scalar);
        if (check != 0)
        {
            ChipLogError(Crypto, "ECDH fallback (%s, status=%ld) invalid private key: %d", reason, statusCode, check);
            mbedtls_mpi_free(&scalar);
            return CHIP_ERROR_INVALID_ARGUMENT;
        }

        result = mbedtls_ecp_mul(&context->curve, (mbedtls_ecp_point *) R, &scalar, &normalizedPoint, CryptoRNG, nullptr);
        ChipLogError(Crypto, "ECDH fallback (%s, status=%ld) mbedtls_ecp_mul=%d", reason, statusCode, result);
        mbedtls_mpi_free(&scalar);
        return (result == 0) ? CHIP_NO_ERROR : CHIP_ERROR_INVALID_ARGUMENT;
    };

    {
        ...
        if (sl_status != SL_STATUS_OK)
        {
            CHIP_ERROR err = fallbackToMbedtls("se-ecdh", static_cast<long>(sl_status));
            mbedtls_ecp_point_free(&normalizedPoint);
            return err;
        }
    }
    ...
    if (status != PSA_SUCCESS)
    {
        CHIP_ERROR err = fallbackToMbedtls("read-x", status);
        mbedtls_ecp_point_free(&normalizedPoint);
        return err;
    }
    ...
    if (status != PSA_SUCCESS)
    {
        CHIP_ERROR err = fallbackToMbedtls("read-y", status);
        mbedtls_ecp_point_free(&normalizedPoint);
        return err;
    }
    ...
    if (status != PSA_SUCCESS)
    {
        CHIP_ERROR err = fallbackToMbedtls("set-z", status);
        mbedtls_ecp_point_free(&normalizedPoint);
        return err;
    }
