...
public:
    void OnIdentifyStart(IdentifyCluster & cluster) override
    {
        ...
        if (identify != nullptr)
        {
            ChipLogError(AppServer, "[IDENTIFY] Codegen OnIdentifyStart endpoint=%u type=%u",
                         static_cast<unsigned>(cluster.GetPaths()[0].mEndpointId), static_cast<unsigned>(identify->mIdentifyType));
    ...
    void OnIdentifyStop(IdentifyCluster & cluster) override
    {
        ...
        if (identify != nullptr)
        {
            ChipLogError(AppServer, "[IDENTIFY] Codegen OnIdentifyStop endpoint=%u",
    ...
    void OnTriggerEffect(IdentifyCluster & cluster) override
    {
        ...
        if (identify != nullptr)
        {
            ChipLogError(AppServer, "[IDENTIFY] Codegen OnTriggerEffect for endpoint %u, effect=%u variant=%u",
                         static_cast<unsigned>(cluster.GetPaths()[0].mEndpointId),
