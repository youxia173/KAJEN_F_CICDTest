...
void PASESession::OnResponseTimeout(ExchangeContext * ec)
{
    ...
    const uint8_t expectedMsgType = to_underlying(mNextExpectedMsg.Value());
    const char * expectedMsgName  = Protocols::GetMessageTypeName(Protocols::SecureChannel::Id, expectedMsgType);
    ChipLogError(SecureChannel,
                 "PASESession timeout waiting peer response. expected=%s(0x%02x)",
                 expectedMsgName, expectedMsgType);
...
CHIP_ERROR PASESession::HandlePBKDFParamRequest(System::PacketBufferHandle && msg)
{
    ...
    if (passcodeId != kDefaultCommissioningPasscodeId)
    {
        ChipLogError(SecureChannel,
                     "PBKDFParamRequest passcodeId mismatch: received=0x%08" PRIx32 " expected=0x%08" PRIx32,
                     static_cast<uint32_t>(passcodeId), static_cast<uint32_t>(kDefaultCommissioningPasscodeId));
    }
...
exit:
    ...
    if (err != CHIP_NO_ERROR)
    {
        const char * currentMsgName = Protocols::GetMessageTypeName(Protocols::SecureChannel::Id, to_underlying(msgType));
        const uint8_t expectedMsgType = mNextExpectedMsg.HasValue() ? to_underlying(mNextExpectedMsg.Value()) : 0xFF;
        const char * expectedMsgName = mNextExpectedMsg.HasValue()
            ? Protocols::GetMessageTypeName(Protocols::SecureChannel::Id, expectedMsgType)
            : "None";

        ChipLogError(SecureChannel,
                     "PASE setup failed: err=%" CHIP_ERROR_FORMAT " current=%s(0x%02x) expected=%s(0x%02x)",
                     err.Format(), currentMsgName, to_underlying(msgType), expectedMsgName, expectedMsgType);
