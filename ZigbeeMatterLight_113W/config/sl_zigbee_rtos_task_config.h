...
// 中文导读（用途）：本文件配置 Zigbee 协议栈与应用框架在 RTOS 下的任务栈和优先级，
// 修改建议：优先通过监控栈水位评估风险，不建议直接大幅调整任务优先级。
// 会影响 Zigbee 消息处理实时性与系统内存占用。
...
#define SL_ZIGBEE_STACK_RTOS_TASK_STACK_SIZE 5800
...
// <i> and will be word aligned when it is applied at the task creation.
#define SL_ZIGBEE_APP_FRAMEWORK_RTOS_TASK_STACK_SIZE 6000
