...
// 中文导读（用途）：本文件是 Zigbee PRO 协议栈的核心容量配置，
// 修改建议：按网络规模逐项调参，避免一次性放大所有表容量导致内存浪费。
// 包括邻居表、绑定表、地址表等关键资源上限。
...
// 中文导读（用途）：这里是 Zigbee 绑定表容量（通常由开关侧/控制端维护绑定关系）。
// 修改建议：开关数量增加前先预留容量，避免后续自动绑定失败或旧项被覆盖。
// 若网络里有更多开关需要点对点绑定到灯，需相应增大该值，避免绑定失败或被覆盖。
